...
typedef struct temp_hum_sensor_data_t {
  uint32_t temperature;
  uint32_t humidity;
} temp_hum_sensor_data_t;
...
void gateway_client_task(void *pvParameter) {
  ...
  for (;;) {
    ...
    case SENSOR_TEM_HUM_DATA:;
      temp_hum_sensor_data_t *temp_hum_sensor_data =
          (temp_hum_sensor_data_t *)sensor_data_queue_message.param;
      ESP_LOGI(TAG, "Sent sensor data: temp=%d hum=%d to the gateway.",
               temp_hum_sensor_data->temperature,
               temp_hum_sensor_data->humidity);
      cJSON *req_body_json;
      ...
      cJSON_AddNumberToObject(req_body_json, "temperature",
                              temp_hum_sensor_data->temperature);
      cJSON_AddNumberToObject(req_body_json, "humidity",
                              temp_hum_sensor_data->humidity);
...
void sensor_task(void *pvParameter) {
  ...
  for (;;) {
    sensor_data_queue_message_t sensor_data_queue_message;
    temp_hum_sensor_data_t *temp_hum_sensor_data =
        malloc(sizeof(temp_hum_sensor_data_t));
    temp_hum_sensor_data->temperature = simulate_rand_sensor(25, 35);
    temp_hum_sensor_data->humidity = simulate_rand_sensor(0, 100);
    sensor_data_queue_message.sensor_data_type = SENSOR_TEM_HUM_DATA;
    sensor_data_queue_message.param = temp_hum_sensor_data;
    xQueueSend(sensor_data_queue, &sensor_data_queue_message, 0);
    ESP_LOGI(TAG, "Sent sensor data: temp=%d hum=%d to queue.",
             temp_hum_sensor_data->temperature, temp_hum_sensor_data->humidity);
